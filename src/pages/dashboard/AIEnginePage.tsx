...
const recentPredictions = [
  {
    id: 1,
    type: "route",
    input: "LA → NYC shipment",
    output: "Optimal route via I-40, ETA 2d 4h",
    confidence: 98.2,
    time: "2s ago",
  },
  {
    id: 2,
    type: "demand",
    input: "Q4 electronics demand",
    output: "+34% surge expected",
    confidence: 94.5,
    time: "5s ago",
  },
  {
    id: 3,
    type: "risk",
    input: "Mediterranean route",
    output: "Low risk (weather clear)",
    confidence: 99.1,
    time: "8s ago",
  },
  {
    id: 4,
    type: "price",
    input: "Shanghai → EU container",
    output: "Optimal price: $2,847",
    confidence: 91.8,
    time: "12s ago",
  },
];
...
export const AIEnginePage = () => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setLiveMetrics((prev) => ({
        predictionsToday:
          prev.predictionsToday + Math.floor(Math.random() * 100),
        ...
        queuedRequests: Math.max(
          0,
          prev.queuedRequests + Math.floor(Math.random() * 20) - 10,
        ),
        gpuUtilization: Math.min(
          100,
          Math.max(50, prev.gpuUtilization + (Math.random() - 0.5) * 5),
        ),
      }));
  ...
  const trainModel = useCallback((modelId: number) => {
    setModels((prev) =>
      prev.map((m) =>
        m.id === modelId
          ? { ...m, status: "training", lastTrained: "Training..." }
          : m,
      ),
    );
    ...
    <div className='space-y-6'>
        ...
        {[
          {
            ...
            value: (liveMetrics.predictionsToday / 1000000).toFixed(2) + "M",
            icon: Brain,
          ...
          {
            label: "Active Models",
            value: liveMetrics.activeModels.toString(),
            icon: Layers,
            color: "text-purple-400",
          },
          {
            label: "Queue",
            value: liveMetrics.queuedRequests.toString(),
            icon: Database,
            color: "text-yellow-400",
          },
          {
            label: "GPU Usage",
            value: liveMetrics.gpuUtilization.toFixed(0) + "%",
            icon: Cpu,
              ...
              >
                <div className='flex items-start justify-between'>
                  ...
                  <div className='text-right'>
                    <p className='text-sm font-bold text-cyber-primary'>
      ...
      >
        <div className='flex items-center justify-between mb-4'>
          ...
          <button
            onClick={() => setIsConfigOpen(true)}
            className='text-sm text-cyber-primary hover:text-accent transition-colors'
          >
            View All →
          </button>
        </div>
            ...
            <motion.div
              ...
              className={`p-4 rounded-xl border cursor-pointer transition-all ${
                selectedModel === model.id
                  ...
                  : "bg-cyber-primary/5 border-cyber-primary/20 hover:border-cyber-primary/30"
              }`}
            >
              <div className='flex items-start justify-between mb-3'>
                <div className='flex items-center gap-2'>
                  <Brain
                    className={`w-5 h-5 ${model.status === "active" ? "text-cyber-primary" : "text-yellow-400"}`}
                  />
                  <span
                ...
                >
                  <RefreshCw
                    className={`w-4 h-4 ${model.status === "training" ? "animate-spin" : ""}`}
                  />
                </button>
              </div>
              ...
              <div className='space-y-4'>
                <div className='flex items-center justify-between'>
                  <div>
                    ...
                    <p className='text-xs text-muted-foreground'>
                      Enable or disable the live processing loop
                    </p>
                  </div>
                  <button
                    onClick={() => {
                ...
                <div className='flex items-center justify-between'>
                  <div>
                    ...
                    <p className='text-xs text-muted-foreground'>
                      Perform bulk actions on models
                    </p>
                  </div>
                  <div className='flex gap-2'>
                    <button
                      onClick={() => {
                        models.forEach((m) => trainModel(m.id));
                        toast({
                          title: "Training",
                          description: "Training started for all models",
                        });
                      }}
                      className='px-3 py-1 rounded-md bg-cyber-primary text-white'
                    >
                    ...
                    <button
                      onClick={() => {
                        ...
                        toast({
                          title: "Reset",
                          description: "Models reset to defaults",
                        });
                      }}
                      className='px-3 py-1 rounded-md bg-cyber-primary/10'
                    >
