...
const aiInsights = [
  {
    type: "trend",
    message: "Revenue up 24% YoY - outperforming market by 12%",
    impact: "+$450K",
    positive: true,
  },
  {
    type: "forecast",
    message: "Q4 demand surge predicted: +34% volume expected",
    impact: "Plan ahead",
    positive: true,
  },
  {
    type: "alert",
    message: "Rising fuel costs impacting margins in EU routes",
    impact: "-2.3%",
    positive: false,
  },
  {
    type: "opportunity",
    message: "Asia Pacific expansion could yield 28% growth",
    impact: "+$1.2M",
    positive: true,
  },
];
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setLiveStats((prev) => ({
        ...
        avgPerformance: Math.min(
          99,
          prev.avgPerformance + (Math.random() - 0.5) * 0.2,
        ),
        costEfficiency: Math.min(
          95,
          prev.costEfficiency + (Math.random() - 0.5) * 0.3,
        ),
        customerSatisfaction: Math.min(
          99,
          prev.customerSatisfaction + (Math.random() - 0.5) * 0.1,
        ),
        onTimeDelivery: Math.min(
          99.5,
          prev.onTimeDelivery + (Math.random() - 0.5) * 0.1,
        ),
      }));
    }, 3000);
    ...
    <div className='space-y-6'>
      ...
      <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4'>
        <div>
          ...
          <p className='text-muted-foreground mt-1'>
            Live dashboards with AI-powered actionable insights
          </p>
        </div>
        <div className='flex items-center gap-3'>
          <div className='flex flex-wrap justify-center items-center gap-1 p-1 rounded-xl bg-cyber-primary/10 border border-cyber-primary/20 '>
          ...
          <button
            ...
            className='p-2 shrink-0 rounded-xl bg-cyber-primary/10 text-cyber-primary hover:bg-cyber-primary/20 transition-colors disabled:opacity-50'
          >
            <RefreshCw
              className={`w-5 h-5 ${isRefreshing ? "animate-spin" : ""}`}
            />
          </button>
          <button className='flex shrink-0 items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-cyber-primary to-accent text-white font-semibold text-sm'>
      ...
      <div className='grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4'>
        {[
          {
            label: "Total Revenue",
            value: `$${(liveStats.totalRevenue / 1000000).toFixed(2)}M`,
            change: "+18%",
            icon: DollarSign,
            positive: true,
          },
          {
            ...
            value: liveStats.totalShipments.toLocaleString(),
            change: "+24%",
            icon: Package,
            positive: true,
          },
          {
            label: "Performance",
            value: liveStats.avgPerformance.toFixed(1) + "%",
            change: "+5%",
            icon: TrendingUp,
            positive: true,
          },
          {
            ...
            value: liveStats.costEfficiency.toFixed(1) + "%",
            change: "+12%",
            icon: BarChart3,
            positive: true,
          },
          {
            label: "Satisfaction",
            value: liveStats.customerSatisfaction.toFixed(1) + "%",
            change: "+3%",
            icon: Users,
            positive: true,
          },
          {
            label: "On-Time",
            value: liveStats.onTimeDelivery.toFixed(1) + "%",
            change: "+2%",
            icon: Clock,
            positive: true,
          },
          ...
          >
            ...
            <div className='flex items-center gap-2 mt-1'>
              ...
              <span
                className={`flex items-center text-xs font-medium ${stat.positive ? "text-green-400" : "text-red-400"}`}
              >
                {stat.positive ? (
          ...
          {aiInsights.map((insight, i) => (
            ...
            >
              <div className='flex items-center justify-between mb-2'>
                ...
                <span
                  className={`text-sm font-bold ${insight.positive ? "text-green-400" : "text-accent"}`}
                >
                  {insight.impact}
                </span>
              </div>
        ...
        >
          <div className='flex items-center justify-between mb-4'>
            ...
            <div className='flex items-center gap-4 text-xs'>
              ...
              <span className='flex items-center gap-1'>
                <span className='w-3 h-3 rounded-full bg-green-400' /> Profit
              </span>
            </div>
          </div>
          <div className='h-64'>
                ...
                {topMetrics.map((metric) => (
                  <tr key={metric.region} className='hover:bg-cyber-primary/5'>
                    ...
                    <td className='py-3 text-muted-foreground'>
                      {metric.shipments.toLocaleString()}
                    </td>
